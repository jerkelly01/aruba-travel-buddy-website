...
} from '/var/task/aruba-travel-buddy-website/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/aruba-travel-buddy-website/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/aruba-travel-buddy-website')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/aruba-travel-buddy-website',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/aruba-travel-buddy-website/.netlify/dist/run/handlers/server.js')
